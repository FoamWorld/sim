## List of license information
- [MIT](./licenses/LICENSE)
- [Apache v2.0](./licenses/LICENSE-Apache)
- [SIL Open Font License (OFL)](https://openfontlicense.org/open-font-license-official-text/): [copyright information not needed](https://openfontlicense.org/how-to-use-ofl-fonts/)
- [CC BY 3.0](https://creativecommons.org/licenses/by/3.0)
- [CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0)
- [CC BY-NC-SA 3.0](https://creativecommons.org/licenses/by-nc-sa/3.0)

## Code
My original. (MIT)

## Fonts
`Open Sans` designed by **Steve Matteson** (Apache v2.0)
- `assets/fonts/open-sans.regular.ttf`: Normal, Weight: 400, Stretch: 100%
- `assets/fonts/open-sans.light-italic.ttf`: Italic, Weight: 300, Stretch: 100%

`Quattrocento` designed by **Pablo Impallari** (OFL)
- `assets/fonts/quattrocento.bold.ttf`: Normal, Weight: 700, Stretch: 100%

## Images
### My Original
- `assets/textures/hints.png`
- `assets/textures/spells.png`
- `assets/textures/wands.png`

### OpenGameArt
[Sign Pack](https://opengameart.org/content/sign-pack) (CC-BY-SA 3.0) (Modified: relocated regions)
- `assets/textures/sign.png`

[Dungeon Tileset](https://opengameart.org/content/dungeon-tileset-1) (CC-BY 3.0) (Modified)
- `assets/textures/character.png`
- `assets/textures/door.png`
